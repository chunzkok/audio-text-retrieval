...
class AudioTextDataCollator:
    def __init__(self, path_to_audio_converter: Callable[[Union[str, List[str]], Union[np.ndarray, List[np.ndarray]]]], k: int):
    ...
    def _random_index_excluding(upper_bound: int, exclude: int, size: Optional[int] = None):
        if size is not None:
